...
@section('title', 'Create - Guides - Staff')

@section('content-staff')
    <div>
        <div class="mb-20 break-words text-center">
            <h2>Guides <span class="font-light">/ Create</span></h2>
        </div>

        <form action="{{ route('staff.guides.store') }}" method="POST">
            @csrf
            ...
            <div class="mb-5 grid grid-cols-6 gap-6 items-end">
                <div class="col-span-full md:col-span-3">
                    ...
                    <input type="text" name="title" id="title-field" class="text-gray-300 bg-gray-800 mt-1 focus:ring-primary-dark focus:border-primary-dark block w-full shadow-sm md:text-sm border-gray-700 rounded-md" value="{{ old('title') }}" required>
                    @error('title')
                ...
                <div class="col-span-full md:col-span-3">
                    ...
                    <input type="text" name="banner_url" id="banner-url-field" class="text-gray-300 bg-gray-800 mt-1 focus:ring-primary-dark focus:border-primary-dark block w-full shadow-sm md:text-sm border-gray-700 rounded-md" value="{{ old('banner_url') }}" required>
                    @error('banner_url')
                        <span class="pt-2 text-sm text-red-500">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="col-span-full">
                    ...
                    <textarea name="content" id="content-input" class="text-gray-300 bg-gray-800 mt-1 focus:ring-primary-dark focus:border-primary-dark block w-full shadow-sm md:text-sm border-gray-700 rounded-md" cols="30" rows="30">{{ old('content') }}</textarea>
                    @error('content')
                        <span class="pt-2 text-sm text-red-500">{{ $message }}</span>
                    @enderror
                </div>
