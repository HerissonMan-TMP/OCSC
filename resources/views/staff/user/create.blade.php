    ...
    <div class="mt-6 px-4 py-5 sm:p-6 bg-gray-800 rounded-md shadow overflow-hidden">
        ...
        <div class="grid grid-cols-6 gap-6">
            <div class="col-span-full">
                ...
                <input type="email" name="email" id="email" class="text-gray-300 bg-gray-700 mt-1 focus:ring-primary-dark focus:border-primary-dark block w-full shadow-sm sm:text-sm border-gray-600 rounded-md" placeholder="myemail@example.com" value="{{ request()->email }}" required>
            </div>

            <div class="col-span-full">
                <label for="name" class="block text-sm font-medium text-gray-300">Name <span class="text-red-500 font-bold">*</span></label>
                <input type="text" name="name" id="name" class="text-gray-300 bg-gray-700 mt-1 focus:ring-primary-dark focus:border-primary-dark block w-full shadow-sm sm:text-sm border-gray-600 rounded-md" placeholder="MyUsername" required>
            </div>
            ...
            <div class="col-span-full sm:col-span-full">
                ...
                <select id="role" name="role_id" class="text-gray-300 bg-gray-700 mt-1 block w-full py-2 px-3 border border-gray-600 bg-white rounded-md shadow-sm focus:outline-none focus:ring-primary-dark focus:border-primary-dark sm:text-sm" required>
                    @foreach($recruitableRoles as $role)
                        <option @if($role->id == request()->role_id) selected @endif value="{{ $role->id }}" style="color: {{ $role->color }}">{{ $role->name }}</option>
                    @endforeach
                </select>
            </div>
        </div>
